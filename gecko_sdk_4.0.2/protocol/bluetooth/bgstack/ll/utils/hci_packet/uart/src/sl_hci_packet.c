...
uint32_t hci_common_transport_transmit(uint8_t *data, int16_t len)
{
  ...
  hci_packet_t tmp;
  memset(&tmp, 0, sizeof(hci_packet_t));
  memcpy(&tmp, data, len);
  if (tmp.packet_type == hci_packet_type_event) {
    if (tmp.hci_evt.eventcode == HCI_Disconnection_Complete) {
      acl_packet_support_counter = 0;
    }

    if(tmp.hci_evt.eventcode == HCI_Command_Complete){
      if (tmp.hci_evt.opcode == HCI_Read_Local_Supported_Commands) {
        tmp.hci_evt.payload[10] |= 0xE0;  //Set bit5,6,7
        memcpy(data, &tmp, len);
      }else if((tmp.hci_evt.opcode == HCI_Set_Host_Controller_To_Host_Flow_Control)
             ||(tmp.hci_evt.opcode == HCI_Host_Buffer_Size)
             ||(tmp.hci_evt.opcode == HCI_Host_Number_Of_Completed_Packets)){
        tmp.hci_evt.status = 0x00;
        memcpy(data, &tmp, len);
      }
    }
  } else if (tmp.packet_type == hci_packet_type_acl_data){
    ...
    if (flow_control.enable){
      //Limit on Host_ACL_Data_Packet_Length
